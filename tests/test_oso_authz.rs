use actix_web::{
    App, HttpResponse, Responder, test,
    web,
};
...
async fn test_oso_authz_success() {
    ...
    oso.load_str(r#"allow(actor, _action, _resource) if actor matches User{name: "alice"};"#).unwrap();

    let authz = OsoAuthorization::with_fn(|req, oso| async move {
        if oso.is_allowed(User { name: "alice".to_string() }, "action", "resource").unwrap() {
            Ok(req)
        } else {
...
async fn test_oso_authz_failure() {
    ...
    let authz = OsoAuthorization::with_fn(|req, oso| async move {
        if oso.is_allowed(User { name: "lice".to_string() }, "action", "resource").unwrap() {
            Ok(req)
        } else {
            Err(actix_web::error::ErrorUnauthorized("no sir"))
        }
